...
def register_cron() -> bool:
    ...
    new_entries = "\n".join([
        ...
        f"@reboot sleep 30 && {py} {agent('company_timekeeper.py')} >> {logf('timekeeper')} 2>&1 &",

        # Patches (Bug Finder) — continuous mode, starts at boot
        f"@reboot sleep 60 && {py} {agent('company_patches.py')} --mode continuous >> {logf('bug_finder')} 2>&1 &",
        ...
        f"0 18 * * 5         {py} {agent('company_blueprint.py')} >> {logf('blueprint')} 2>&1",
