...
REQUIRED_AGENT_FILES = [
    "company_patches.py",
    "company_blueprint.py",
    "company_sentinel.py",
    "company_fidget.py",
    "company_librarian.py",
    "company_scoop.py",
    "company_vault.py",
    "company_timekeeper.py",
...
def register_cron() -> bool:
    ...
    new_entries = "\n".join([
        ...
        # NOTE 2026-05-09: company_timekeeper, company_patches, and
        # company_blueprint were superseded by other systems; their cron
        # entries previously here were stale (pointed at non-existent
        # files). Removed by agent-heartbeat audit.

        # Sentinel — runs every 15 minutes during market hours
        f"*/15 9-16 * * 1-5  {py} {agent('company_sentinel.py')} >> {logf('sentinel')} 2>&1",

        # Scoop — runs every 10 minutes (delivery queue drain)
        f"*/10 * * * *       {py} {agent('company_scoop.py')} >> {logf('scoop')} 2>&1",

        # Vault — hourly key compliance check
        f"0 * * * *          {py} {agent('company_vault.py')} >> {logf('vault')} 2>&1",

        # Fidget (token monitor) — daily at 8am ET
        f"0 8 * * *          {py} {agent('company_fidget.py')} >> {logf('fidget')} 2>&1",

        # Librarian — weekly Sunday 9am ET
        f"0 9 * * 0          {py} {agent('company_librarian.py')} >> {logf('librarian')} 2>&1",

        # Strongbox (Backup Manager) — nightly 11pm ET
        f"0 23 * * *         {py} {agent('company_strongbox.py')} >> {logf('strongbox')} 2>&1",

        # node_heartbeat — outbound system-metrics POST to MONITOR_URL every 5min
        f"*/5 * * * *        {py} {str(SYNTHOS_HOME / 'node_heartbeat.py')} >> {logf('node_heartbeat')} 2>&1",

        # Saturday maintenance reboot — 4am ET
        f"0 4 * * 6  sudo reboot",
