"""
Unit tests for bias_audit_agent.run_agent().

No DB, network, or file I/O is performed.
bias_audit_agent calls DB() at module level; the DB class is replaced with
...
V1 classification values:
    pass | pass_with_bias_warning | fairness_review_recommended |
    fail_bias_audit | block_output
"""
...
if "bias_audit_agent" not in sys.modules:
...
from bias_audit_agent import run_agent  # noqa: E402

# ── Valid classification set (V1) ───────────────────────────────────────────────
_VALID_CLASSIFICATIONS = {
    "pass",
    "pass_with_bias_warning",
    "fairness_review_recommended",
    "fail_bias_audit",
    "block_output",
}
...
def test_null_trade_output_blocks():
    """trade_output=None → classification is in the V1 set (gate-1 halt maps to fail_bias_audit)."""
...
def test_bias_findings_is_list():
    """bias_findings is always a list regardless of input."""
    snap = _snapshot()
    result = run_agent(snap)
    assert isinstance(result.get("bias_findings"), list)
...
def test_classification_in_valid_set():
    """classification is one of the five valid V1 values."""
    snap = _snapshot()
    result = run_agent(snap)
    assert result.get("classification") in _VALID_CLASSIFICATIONS
